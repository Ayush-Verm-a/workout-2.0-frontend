...
const LiveWorkoutComponent = () => {
  ...
  const deleteSet = (exerciseIndex, setIndex) => {
    setActiveExercises((draft) => {
      ...
      draft[exerciseIndex].sets.forEach((set, i) => (set.setNumber = i + 1));
    });
  };
  ...
  const addExerciseToWorkout = (defId) => {
    ...
    const exists = activeExercises.findIndex((ex) => ex.definitionId === defId);
        ...
        <div className="live-workout__header-right">
          {!isActive ? (
            ...
            <>
              ...
              <button onClick={finishWorkout} className="btn-finish">
                <Save /> Finish
              </button>
            </>
          )}
            ...
            <div key={exerciseIndex} className="exercise-item">
              <div className="exercise-item__header">
                <h3 className="exercise-item__title">
                  <span>{exercise.name.substring(0, 2).toUpperCase()}</span>
                  {exercise.name}
                </h3>
                ...
                <div className="exercise-item__controls">
                  <div className="calorie-input">
                    ...
                    <input
                      ...
                      onChange={(e) => handleAddCalories(e, exerciseIndex)}
                    />
                    <span>cal</span>
                  </div>
                    ...
                    <div className="set-item" key={index}>
                      ...
                      <div className="set-item__input">
                        <input
                          ...
                          value={set.weight}
                          onChange={(e) =>
                            updateSet(e, exerciseIndex, index, "weight")
                          }
                        />
                      </div>
                      <div className="set-item__input">
                        <input
                          ...
                          value={set.reps}
                          onChange={(e) =>
                            updateSet(e, exerciseIndex, index, "reps")
                          }
                        />
                      </div>
                      <div className="set-item__remove">
                        <button onClick={() => deleteSet(exerciseIndex, index)}>
                          <X />
                        </button>
                      </div>
                    </div>
